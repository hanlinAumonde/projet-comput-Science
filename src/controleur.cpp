...
TypeLitterale Controleur::estLitterale(const QString &s)
{
    ...
    if (s[0] >= 'A' && s[0] <= 'Z' && estOperateurPredefini(s) == false)
    {
        return TypeLitterale::ATOME;
    }
...
void Controleur::traiterCommandBar(const QString &s)
{
    ...
    identifieurMap_t identifieurMapCopy{identifieurMap_};
    try
    {
        ...
        for (size_t i = 0; i < listOperande.size(); i++)
        {
            ...
            if (type != TypeLitterale::NONE && !estIdentifieur(operande)) // littérale entière || réelle || expression || programme || atome (hors identifieur)
            {
        ...
        if (listOperande.size() != 0 && (listOperande[0] != "UNDO" && listOperande[0] != "REDO"))
        {
            ...
            historyIndex_++;
            MementoPile *mementoPile = new MementoPile(*litteraleAffiche_->cloneOnHeapGeneral(), identifieurMap_);
    ...
    catch (const std::runtime_error &e)
    {
        ...
        identifieurMap_ = identifieurMapCopy;
        throw;
    }
}
...
void Controleur::loadPrecedent()
{
    if (historyIndex_ > 0)
    {
        ...
        identifieurMap_ = careTakerPile_->getMementoPile(historyIndex_).getIdentifieurMap();
    }
}
