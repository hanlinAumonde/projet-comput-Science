...
{
    if (operateur == "ARCSIN" || operateur == "ARCCOS") {
        if(valeur_<-1 || valeur_>1){
            throw CalculateurException(("Argument invalide : " +
                                        affichage().toStdString() + " a dépassé l'intervalle entre -1 et 1 !")
                                           .c_str());
        }
    }
